<x-dashboard.layout title_header="Users">
    <div class="px-4 py-6 m-5 bg-white shadow rounded-xl">
        <div class="flex items-center justify-between">
            <h1 class="text-2xl font-bold tracking-wide uppercase">Users</h1>
                ...
                <tbody>
                    @foreach ($users as $user)
                        <tr class="bg-white border-b border-gray-200 hover:bg-gray-50 ">
                            ...
                            <td class="px-6 py-4">
                                <span
                                    class=" text-xs font-medium me-2 px-2.5 py-0.5 rounded-sm border  uppercase {{ $user->role == 'admin' ? 'bg-red-100 text-red-800 border-red-400' : 'bg-green-100 text-green-800 border-green-400' }}">{{ $user->role }}</span>
                            </td>
                            <td class="px-6 py-4">
                                <button type="button" id="DropdownAll-button-{{ $user->id }}"
                                    data-dropdown-toggle="DropdownMenu-{{ $user->id }}" class="cursor-pointer">
                                ...
                                <div id="DropdownMenu-{{ $user->id }}"
                                    ...
                                    <ul class="py-2 text-sm text-gray-700" aria-labelledby="dropdownDefaultButton">
                                        <li>
                                            {{-- button edit --}}
                                            <button type="button" data-modal-target="edit-user-{{ $user->id }}"
                                                data-modal-toggle="edit-user-{{ $user->id }}"
...
{{-- modal --}}
<x-modal.admin.add_user />
@foreach ($users as $user)
    <x-modal.admin.edit_user :id_user="$user->id" :user="$user" />
@endforeach
